...
public:
  ...
  inline void process(float x, float &low, float &band, float &high) noexcept {
    ...
    if (std::fabs(mIc1eq) < 1e-20f)
      mIc1eq = 0.f;
    if (std::fabs(mIc2eq) < 1e-20f)
      mIc2eq = 0.f;
  }
...
template <typename T> class ParamMailbox {
public:
...
public:
  struct Params {
    ...
    bool rejectR = false;
    float mix = 1.0f;
    float randomAmount[4] = {0.1f, 0.1f, 0.1f, 0.1f};
    double randomPeriodSec[4] = {1.0, 1.0, 1.0, 1.0};
    bool randomEnableFreqL = false, randomEnableBwL = false, randomEnableGainL = false;
    bool randomEnableFreqR = false, randomEnableBwR = false, randomEnableGainR = false;
    bool randomEnableMix = false;
    std::uint8_t randomColorFreqL = 0, randomColorBwL = 0, randomColorGainL = 0;
    std::uint8_t randomColorFreqR = 0, randomColorBwR = 0, randomColorGainR = 0;
    std::uint8_t randomColorMix = 0;
  ...
  const RandomDeltas &randomDeltas() const noexcept { return mRandomDeltas; }
  ...
  void prepare(double sampleRate) noexcept {
    ...
    for (int i = 0; i < kNumRandomStreams; ++i) {
      mRandomStream[i].walk.seed(0x9E3779B9u * (unsigned)(i + 1) + 0x5bd1e995u);
      mRandomStream[i].walk.setPeriod(mRandomStream[i].periodSec, sampleRate);
    }
    reset();
  }
  void reset() noexcept {
  ...
  void setParams(const Params &p) noexcept {
    ...
    for (int c = 0; c < 4; ++c) {
      mParams.randomAmount[c] = std::clamp(p.randomAmount[c], 0.0f, 1.0f);
      mParams.randomPeriodSec[c] = std::max(p.randomPeriodSec[c], 0.001);
    }
    if (p.linked) {
      ...
      mParams.randomEnableFreqR = mParams.randomEnableFreqL;
      mParams.randomColorFreqR = mParams.randomColorFreqL;
      mParams.randomEnableBwR = mParams.randomEnableBwL;
      mParams.randomColorBwR = mParams.randomColorBwL;
      mParams.randomEnableGainR = mParams.randomEnableGainL;
      mParams.randomColorGainR = mParams.randomColorGainL;
    }
    // 必须从镜像后的 mParams 读取（而非原始 p）—— linked 时 R 流的 enable/color 已在上方
    // 被强制同步为 L 值，若此处读 p 会形成两条不一致的数据来源。
    const struct {
      bool en;
      std::uint8_t color;
    } maps[kNumRandomStreams] = {
        {mParams.randomEnableFreqL, mParams.randomColorFreqL}, {mParams.randomEnableBwL, mParams.randomColorBwL},
        {mParams.randomEnableGainL, mParams.randomColorGainL}, {mParams.randomEnableFreqR, mParams.randomColorFreqR},
        {mParams.randomEnableBwR, mParams.randomColorBwR},     {mParams.randomEnableGainR, mParams.randomColorGainR},
        {mParams.randomEnableMix, mParams.randomColorMix},
    };
    for (int i = 0; i < kNumRandomStreams; ++i) {
      const int col = std::min((int)maps[i].color, 3);
      const float amt = mParams.randomAmount[col];
      mRandomStream[i].active = maps[i].en && amt > 0.0f;
      mRandomStream[i].amount = amt;
      mRandomStream[i].periodSec = mParams.randomPeriodSec[col];
      mRandomStream[i].walk.setPeriod(mRandomStream[i].periodSec, mSampleRate);
    }
  ...
  void process(const float *const inL, const float *const inR, float *const outL, float *const outR, int n,
  ...
  void process(const float *const in, float *const out, int n, float *wetOut = nullptr) noexcept {
...
private:
  // 随机调制幅度上限（满量程时的最大偏移）
  static constexpr double kRandomFreqModOct = 4.90689059560852; // = log2(30): 中心频率 ±30 倍
  static constexpr double kRandomBwModOct = 4.64385618977472;   // = log2(25): 带宽 ±25 倍
  static constexpr float kRandomGainModDb = 48.f;               // 增益 ±48 dB
  static constexpr float kRandomMixMod = 0.5f;                  // 干湿 ±50%
  static constexpr float kDbToLin = 0.115129254649702f;         // = ln(10)/20: dB -> 线性
  ...
  public:
    ...
    void setPeriod(double seconds, double sampleRate) noexcept { mInc = 1.0 / std::max(seconds * sampleRate, 1.0); }
  ...
  void run(const float *const *in, float *const *out, float *const *wet, int numCh, int n) noexcept {
    const float gains[2] = {mParams.gainL, mParams.gainR};
    const bool mixMod = mRandomStream[kRandomMix].active;
    ...
    for (int i = 0; i < n; ++i) {
      ...
      if (mRandomStream[kRandomFreqL].active)
        freqOct[0] =
            mRandomStream[kRandomFreqL].walk.tick() * mRandomStream[kRandomFreqL].amount * (float)kRandomFreqModOct;
      if (mRandomStream[kRandomBwL].active)
        bwOct[0] = mRandomStream[kRandomBwL].walk.tick() * mRandomStream[kRandomBwL].amount * (float)kRandomBwModOct;
      if (mRandomStream[kRandomGainL].active) {
        gainDb[0] = mRandomStream[kRandomGainL].walk.tick() * mRandomStream[kRandomGainL].amount * kRandomGainModDb;
      ...
      if (numCh > 1) {
        if (mParams.linked) {
          ...
          if (mRandomStream[kRandomFreqR].active)
            freqOct[1] =
                mRandomStream[kRandomFreqR].walk.tick() * mRandomStream[kRandomFreqR].amount * (float)kRandomFreqModOct;
          if (mRandomStream[kRandomBwR].active)
            bwOct[1] =
                mRandomStream[kRandomBwR].walk.tick() * mRandomStream[kRandomBwR].amount * (float)kRandomBwModOct;
          if (mRandomStream[kRandomGainR].active) {
            gainDb[1] = mRandomStream[kRandomGainR].walk.tick() * mRandomStream[kRandomGainR].amount * kRandomGainModDb;
            gainLin[1] = std::exp(gainDb[1] * kDbToLin);
          }
        }
      }
      if (mixMod) {
        mixDelta = mRandomStream[kRandomMix].walk.tick() * mRandomStream[kRandomMix].amount * kRandomMixMod;
      ...
      mRandomDeltas.freqOct[0] = freqOct[0];
      mRandomDeltas.freqOct[1] = freqOct[1];
      mRandomDeltas.bwOct[0] = bwOct[0];
      mRandomDeltas.bwOct[1] = bwOct[1];
      mRandomDeltas.gainDb[0] = gainDb[0];
      mRandomDeltas.gainDb[1] = gainDb[1];
      mRandomDeltas.mix = mixDelta;
    }
  }
  ...
  Params mParams;
  Channel mCh[2];
  RandomStream mRandomStream[kNumRandomStreams];
  RandomDeltas mRandomDeltas;
};
