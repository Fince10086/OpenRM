...
private:
  ...
  void AddBand(double fc, double bw, double fs, int L, int nextDeep, double fSeg) {
    ...
    for (int n = 0; n < wl; ++n) {
      ...
      if (mWindowType == 0) {
        // 0: Hann 窗 (锐利档): 窄主瓣, 提升相邻音符分辨力与时域起振敏锐度
        w = 0.5 * (1.0 - std::cos(theta));
      } else {
        // 1: 5-term Blackman-Harris 窗 (纯净档): 阻带衰减至 -125dB (数值优化系数),
        // 主瓣半宽 ~5 bin, 纯净度换少许分辨力
  ...
  int mWindowType = 0;                    // 窗函数档位 (0=锐利/Hann, 1=纯净/5阶BH)
  int mBpo = 24;                          // bins per octave (插件层固定 24)
  int mGamma = 5;                         // 低频带宽下限 Hz (插件层按档位下发, 默认高档 5Hz, 越小越精细)
