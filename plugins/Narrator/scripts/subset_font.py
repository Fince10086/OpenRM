...
def main():
    ...
    input_files = [strings_file]
    candidate_ctrl = os.path.join(os.path.dirname(strings_file), "controls", "CandidatePanelControl.h")
    if os.path.exists(candidate_ctrl):
        input_files.append(candidate_ctrl)

    charset = collect_charset(input_files)
    print(f"subset_font.py: 中文字符 {len(charset)} 个 (来自 {', '.join(os.path.basename(p) for p in input_files)})")
