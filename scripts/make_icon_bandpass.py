...
用途:
  本脚本为 BandPass 插件生成 ORMBandPass.icns (macOS) 与 ORMBandPass.ico (Windows)。
  做下一个插件时, 复制本脚本并修改下方 PLUGIN 段 (名字/输出文件名) 即可复用。

特性:
  - 主图: 1024x1024, macOS Big Sur 规范圆角矩形 (radius 185), 白底 #FFFFFF, 黑字 #000000
  - 文字: 插件粗体字体 (Mixed-Bold.ttf, Outfit Bold 子集), 两行品牌名 (ORM / BandPass),
          两行同字号粗体、左对齐、整体垂直居中; 水平以最宽一行 (BandPass) 为基准左右对称
  - 输出: {ICON_NAME}.icns (iconutil) 与 {ICON_NAME}.ico (Win 多尺寸)

用法: python make_icon_bandpass.py <resources_dir> <bold_ttf> <out_png_dir>
示例: python make_icon_bandpass.py plugins/BandPass/resources \
        plugins/BandPass/resources/fonts/Mixed-Bold.ttf /tmp/icon_out
"""
...
# ---- PLUGIN 段: 做新插件时改这里 ----
ICON_NAME = "ORMBandPass"    # 输出文件名 (icns/ico 均用此名, 与 iPlug2 自动发现规则一致)
TEXT_TOP = "ORM"             # 第一行
TEXT_BOTTOM = "BandPass"     # 第二行
# -----------------------------------
