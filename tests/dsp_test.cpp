...
int main() {
  printf("=== BandPassCore v%s 自测 (fs=%d) ===\n", PLUG_VERSION_STR, (int)kFs);
  ...
  // ---- 3) link: R 跟随 L ----
  {
    ...
    for (int i = 0; i < n; ++i)
      in[i] = std::sin(2.0 * M_PI * 3000.0 * i / kFs) * 0.5f + std::sin(2.0 * M_PI * 500.0 * i / kFs) * 0.5f;
    for (int pos = 0; pos < n; pos += kBlock) {
      const int b = std::min(kBlock, n - pos);
      core.updateSmoothing(b);
  ...
  // ---- 6) 稳定性: 白噪声高 Q + 全部随机映射开启 ----
  {
    ...
    p.freqR = 250.0;
    p.bwR = 0.1;
    p.randomAmount[0] = 0.5f;
    p.randomAmount[1] = 1.0f;
    p.randomAmount[2] = 0.3f;
    p.randomAmount[3] = 0.7f;
    p.randomPeriodSec[0] = 0.125;
    p.randomPeriodSec[1] = 2.0;
    p.randomPeriodSec[2] = 0.02;
    p.randomPeriodSec[3] = 10.0;
    p.randomEnableFreqL = true;
    p.randomEnableBwL = true;
    p.randomEnableGainL = true;
    p.randomEnableFreqR = true;
    p.randomEnableBwR = true;
    p.randomEnableGainR = true;
    p.randomEnableMix = true;
    p.randomColorFreqL = 0;
    p.randomColorBwL = 1;
    p.randomColorGainL = 2;
    p.randomColorFreqR = 3;
    p.randomColorBwR = 2;
    p.randomColorGainR = 1;
    p.randomColorMix = 0;
    core.setParams(p);
    ...
    for (int i = 0; i < n; ++i) {
      ...
      in[i] = (float)(s / 4294967296.0 * 2.0 - 1.0);
    }
    for (int pos = 0; pos < n; pos += kBlock) {
      const int b = std::min(kBlock, n - pos);
      core.updateSmoothing(b);
    ...
    double peak = 0.0;
    for (int i = 0; i < n; ++i) {
      if (!std::isfinite(out[i]) || std::fabs(out[i]) > 16.0f) {
        stable = false;
        break;
      }
  ...
  // ---- 6b) 随机映射确实改变输出: CENTER 满量程 + GAIN 满量程的极端影响 ----
  {
    ...
    pB.randomEnableFreqL = true;
    pB.randomColorFreqL = 0;
    pB.randomAmount[0] = 1.0f;
    pB.randomPeriodSec[0] = 0.02; // 快速游走, RMS 平均化后可见差异
    coreB.setParams(pB);
    ...
    pC.randomEnableGainL = true;
    pC.randomColorGainL = 0;
    pC.randomAmount[0] = 1.0f;
    pC.randomPeriodSec[0] = 0.02;
    coreC.setParams(pC);
    ...
    pD.randomEnableMix = true;
    pD.randomColorMix = 0;
    pD.randomAmount[0] = 1.0f;
    pD.randomPeriodSec[0] = 0.02;
    coreD.setParams(pD);
  ...
  // ---- 6c) 宿主 Reset 后随机游走必须继续推进 ----
  {
    ...
    p.randomEnableFreqL = true;
    p.randomColorFreqL = 0;
    p.randomAmount[0] = 1.0f;
    p.randomPeriodSec[0] = 0.01f; // 快速游走
    core.setParams(p);
    ...
    for (int pos = 0; pos < n; pos += kBlock) {
      ...
      core.updateSmoothing(b);
      core.process(in.data() + pos, out.data() + pos, b);
      maxDelta = std::max(maxDelta, std::fabs(core.randomDeltas().freqOct[0]));
    }
  ...
  // ---- 7) slope: 默认 96; -3dB 带宽跨档位保持; 越高滚降越陡 ----
  {
    ...
    auto bwAt = [](double slopeDb) {
      ...
      for (int it = 0; it < 50; ++it) {
        const double mid = 0.5 * (lo + hi);
        if (measureGain(core, mid) > 0.7071)
          hi = mid;
        else
          lo = mid;
      }
    ...
    check("slope: -3dB BW preserved across 12/24/48/96", std::fabs(bw96 - bw12) / bw12 < 0.25 &&
  ...
  // ---- 8) 左右斜率独立 (LINK 关) / R 跟随 L (LINK 开) ----
  {
    ...
    p.linked = true;
    p.slopeDbR = 96.0; // R 参数仍为 96, LINK 时核心应强制跟随 L
    core.setParams(p);
    double l2, r2;
    runDb(l2, r2);
